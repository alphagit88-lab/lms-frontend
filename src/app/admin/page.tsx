                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-1 border-b border-slate-200 mb-2">
                            <button
                                ...
                                className={`flex items-center gap-2 px-5 py-3 text-sm font-semibold transition border-b-2 -mb-px ${txSubTab === 'payhere' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-700'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-5 py-3 text-sm font-semibold transition border-b-2 -mb-px ${txSubTab === 'bank-transfer'
                                            ...
                                            {payments.filter(p => p.paymentMethod !== 'bank_transfer').length === 0 ? (
                                                ...
                                                payments.filter(p => p.paymentMethod !== 'bank_transfer').map((p) => (
                                                    <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50 transition">
                                                        ...
                                                        <td className="px-5 py-4">
                                                            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${p.paymentStatus === 'completed' ? 'bg-emerald-100 text-emerald-700' :
                                                        ...
                                                        <td className="px-5 py-4 text-right">
                                                            {p.paymentStatus === 'pending' && (
                                                                <div className="flex items-center justify-end gap-2">
                                                                    ...
                                                                    <button onClick={() => handleCancelPayment(p.id)} disabled={actionLoading === p.id} className="px-3 py-1 border border-red-200 text-red-600 rounded-lg text-[10px] font-bold hover:bg-red-50 disabled:opacity-50">CANCEL</button>
                                                                </div>
                                                            )}
                                                ...
                                                <tbody>
                                                    {manualPayments.map((p) => (
                                                        <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50 transition">
                                                            ...
                                                            <td className="px-5 py-4">
                                                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${p.paymentStatus === 'under_review' ? 'bg-amber-100 text-amber-700' :
                                                            ...
                                                            <td className="px-5 py-4">
                                                                {p.bankSlipUrl ? (
                                                                    <a
                                                                        href={`${typeof window !== "undefined" ? "" : (process.env.NEXT_PUBLIC_API_URL?.replace("/api", "") || "http://localhost:3001")}${p.bankSlipUrl}`}
                                                                        target="_blank"
