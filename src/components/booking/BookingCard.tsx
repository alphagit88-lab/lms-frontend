...
export default function BookingCard({
  ...
  return (
    <div
      className={`bg-white rounded-lg border p-4 sm:p-5 transition ${(booking.status === 'pending' || booking.status === 'pending_payment')
      ...
      <div className="flex flex-col sm:flex-row sm:items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm">
            ...
            {/* Meeting Link */}
            {(booking.meetingStartLink || booking.meetingLink) && booking.status === 'confirmed' && (
              <div className="flex items-center gap-2 text-gray-600">
                <svg className="w-4 h-4 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                </svg>
                <a
                  href={booking.meetingStartLink || booking.meetingLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`${booking.meetingStartLink ? "text-emerald-600 font-medium" : "text-blue-600"} hover:underline truncate`}
                >
                  {booking.meetingStartLink ? "Start Meeting (Host)" : "Join Meeting"}
                </a>
              </div>
            )}
          </div>
          ...
          {booking.status === 'pending_payment' && (
            <div className="mt-3 bg-purple-50 rounded-md px-3 py-2 text-sm text-purple-700 flex items-center gap-2">
              <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              Student has initiated payment — waiting for payment confirmation.
            </div>
          )}
