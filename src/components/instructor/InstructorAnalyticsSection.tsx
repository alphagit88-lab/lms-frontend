...
export default function InstructorAnalyticsSection() {
  ...
  return (
    <section className="space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-widest">Analytics & Insights</h2>
          <p className="text-xs text-slate-400 mt-0.5">Track student progress, course performance, and earnings</p>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-800">Earnings Over Time</h3>
            ...
            <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div className="relative">
                ...
                >
                  ...
                  {courses.map((c, i) => (
                    <option key={c.id ?? i} value={c.id}>{c.title}</option>
                  ))}
                </select>
                ...
                {students.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {students.map((s, i) => (
                        <tr key={s.enrollmentId ?? `student-${i}`} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            ...
                            <p className="text-xs text-slate-400 mt-1">{s.completedLessons}/{s.totalLessons} lessons</p>
                          </td>
                          <td className="px-6 py-4 text-right">
                            {s.avgExamScore != null ? (
                              <span className={`font-bold ${s.avgExamScore >= 70 ? "text-emerald-600" : s.avgExamScore >= 50 ? "text-amber-600" : "text-red-500"}`}>
                                {s.avgExamScore}%
                              </span>
              ...
              <div className="overflow-x-auto">
                {attendance.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {attendance.map((a, i) => (
                        <tr key={a.studentId ?? `attendance-${i}`} className="hover:bg-slate-50 transition-colors">
                          <td className="px-6 py-4">
              ...
              <div className="p-6">
                {!selectedCourse ? (
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <h4 className="font-bold text-slate-700 text-sm">Top Performers</h4>
                        </div>
                        {coursePerf.topPerformers.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {coursePerf.topPerformers.map((s, i) => (
                              <div key={s.studentId ?? `top-${i}`} className="flex items-center gap-3 bg-amber-50 rounded-xl px-4 py-2">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <h4 className="font-bold text-slate-700 text-sm">Needs Attention (&lt;50%)</h4>
                        </div>
                        {coursePerf.needsAttention.length === 0 ? (
                          <p className="text-xs text-slate-400">All students are passing!</p>
                        ) : (
                          <div className="space-y-2">
                              {coursePerf.needsAttention.map((s, i) => (
                              <div key={s.studentId ?? `needs-${i}`} className="flex items-center gap-3 bg-red-50 rounded-xl px-4 py-2">
                                <p className="flex-1 text-sm font-semibold text-slate-700">{s.name}</p>
                    ...
                    <div>
                      <h4 className="font-bold text-slate-700 text-sm mb-3">Lesson Engagement</h4>
                      {coursePerf.lessonEngagement.length === 0 ? (
                        <p className="text-xs text-slate-400">No lessons in this course yet.</p>
                      ) : (
                        <div className="space-y-3">
                          {coursePerf.lessonEngagement.map((l, i) => (
                            <div key={l.lessonId ?? `lesson-${i}`}>
